...
I applied Unet (Convolution) based on this paper https://arxiv.org/abs/1505.04597
...
train_images = np.zeros(shape=(25,256,256,1))
train_labels = np.zeros(shape=(25,256,256,1))
test_images  = np.zeros(shape=(10,256,256,1))
...
train_images = (train_images - np.nanmin(train_images)) / (np.nanmax(train_images) - np.nanmin(train_images))
train_labels = (train_labels - np.nanmin(train_labels)) / (np.nanmax(train_labels) - np.nanmin(train_labels))
test_images = (test_images - np.nanmin(test_images)) / (np.nanmax(test_images) - np.nanmin(test_images))
...
with tf.Session() as sess:
    ...
    for iter in range(num_epoch):
        ...
        # train
        for current_batch_index in range(0,len(train_images)-2,batch_size):
        ...
        if iter % 2 == 0:
            ...
            plt.savefig('train_change/'+str(iter)+"_Original_Image.png")

            plt.figure()
            plt.imshow(np.squeeze(test_example_gt),cmap='gray')
            plt.axis('off')
            plt.title('Ground Truth Mask')
            plt.savefig('train_change/'+str(iter)+"_Original_Mask.png")

            plt.figure()
            plt.imshow(np.squeeze(sess_results),cmap='gray')
            plt.axis('off')
            plt.title('Generated Mask')
            plt.savefig('train_change/'+str(iter)+"_Generated_Mask.png")
            plt.close('all')
    ...
    for data_index in range(0,len(train_images),batch_size):
        ...
        plt.close('all')
    # save mode for later!
    save_path = saver.save(sess, "/home/bkd/Downloads/code/model.ckpt")
    print("Model saved in path: %s" % save_path)
    for data_index in range(0,len(test_images),batch_size):
        print('data_index',data_index)
        sess_results = sess.run(layer10,feed_dict={x:test_images[data_index:data_index+batch_size,:,:,:]})
        sess_results = sess_results[0,:,:,:]
        test_example =   test_images[data_index,:,:,:]
        #test_example = test_example[0,:,:,:]
        plt.figure()
        plt.imshow(np.squeeze(sess_results),cmap='gray')
        plt.axis('off')
        plt.title('Generated Mask')
        plt.savefig('predictions/'+str(data_index)+"_Generated_Mask.png")
 
        plt.figure()
        plt.imshow(np.squeeze(test_example),cmap='gray')
        plt.axis('off')
        plt.title('Original Image')
        plt.savefig('predictions/'+str(data_index)+"_Original_Image.png")
        plt.close('all')
